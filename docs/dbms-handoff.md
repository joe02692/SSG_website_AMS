...
**From:** Backend team
**Date:** 27 Aug 2026
**Ask:** migration SQL files (`supabase/migrations/0004_*.sql` onward, one per change set) added to the repo, following the conventions in §2 below.

**Updated 27 Aug 2026:** backend has since added `0002` and `0003`, which split the old single
`leader` role into `stage_admin` and `stage_leader`. §1 below reflects the current state — please
re-read it if you started from an earlier copy of this document.
...
Migrations `0001`–`0003` already created:

- `public.user_role` — enum: `scout | parent | stage_admin | stage_leader` (plus a dead `leader`
  value that predates the admin/leader split; every row was migrated off it in `0003` and nothing
  is ever assigned it now — Postgres just can't drop an enum value).
...
- `public.is_leader()` — SECURITY DEFINER helper, true for any leader-level role. **Use this in new
  ...
  deliberately identical in permissions today and will diverge later, and this function is the one
  place that should need changing when they do.

Both helpers exist so RLS policies can check the caller's role without recursing on `profiles` —
reuse them rather than re-querying `profiles` from inside a `profiles` policy.
